...
impl RustfsAdminClient {
    ...
    /// Create a user, or reset an existing user's password (RustFS's
    /// MinIO-admin-compatible `add-user` is an upsert: PUT-ing an existing
    /// access key resets its secret key). Callers rely on this to rotate a
    /// user's password atomically when the driver's local credential Secret
