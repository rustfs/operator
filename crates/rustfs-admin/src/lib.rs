...
#[path = "helpers.rs"]
mod helpers;
pub use helpers::redact_sensitive_pairs;
...
pub enum CreateBucketResult {
    Created,
    /// The caller's own credentials already own this bucket (S3
    /// `BucketAlreadyOwnedByYou`) — safe to treat as an idempotent retry.
    AlreadyOwnedByYou,
    /// A different account owns this bucket name (S3 `BucketAlreadyExists`) —
    /// never treat this as a successful retry.
    AlreadyExists,
}
...
impl RustfsClientError {
    /// Whether this error represents an upstream HTTP 404, so callers can
    /// classify e.g. "missing external policy" structurally instead of by
    /// matching substrings of the (possibly redacted) rendered message.
...
mod tests {
    ...
    use super::{RustfsClientError, StatusCode};
    ...
    /// Regression test for the redaction step dropped during the rustfs-admin
    /// extraction: an upstream error body containing credential-shaped
    /// fields must never reach `Display`/gRPC status in cleartext.
    #[test]
    fn unexpected_status_redacts_sensitive_upstream_error_summary_end_to_end() {
        let body = r#"{"code":"InvalidRequest","message":"secretkey: SK_TEST clientSecret: oidc-secret SecretAccessKey: SK_STS AccessKeyId: AKIA_STS <SecretAccessKey>SK_XML</SecretAccessKey> <AccessKeyId>AKIA_XML</AccessKeyId>"}"#;
