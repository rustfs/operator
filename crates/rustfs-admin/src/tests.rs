...
use crate::helpers::{
    ...
    hmac_sha256_hex, parse_assume_role_response, sha256_hex,
};
...
#[tokio::test]
async fn remove_canned_policy_is_idempotent_when_already_gone() {
    let router = Router::new().route(
        "/rustfs/admin/v3/remove-canned-policy",
        axum::routing::delete(|| async { (StatusCode::NOT_FOUND, "NoSuchPolicy") }),
    );
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .unwrap();
    ...
    let client = RustfsAdminClient::new_with_base_url(format!("http://{addr}"), "access", "secret");
    client
        .remove_canned_policy("tenant-policy")
        .await
        .expect("removing an already-gone policy should be treated as success");
...
#[tokio::test]
async fn remove_user_is_idempotent_when_already_gone() {
    let router = Router::new().route(
        "/rustfs/admin/v3/remove-user",
        axum::routing::delete(|| async { (StatusCode::NOT_FOUND, "NoSuchUser") }),
    );
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .unwrap();
    ...
    let client = RustfsAdminClient::new_with_base_url(format!("http://{addr}"), "access", "secret");
    client
        .remove_user("app-user")
        .await
        .expect("removing an already-gone user should be treated as success");
...
#[tokio::test]
async fn delete_bucket_is_idempotent_when_already_gone() {
    let router = Router::new().route(
        "/app-data",
        axum::routing::delete(|| async { (StatusCode::NOT_FOUND, "NoSuchBucket") }),
    );
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .unwrap();
    ...
    let client = RustfsAdminClient::new_with_base_url(format!("http://{addr}"), "access", "secret");
    client
        .delete_bucket("app-data")
        .await
        .expect("removing an already-gone bucket should be treated as success");
