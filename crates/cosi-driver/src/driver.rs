...
fn map_bucket(err: BucketError) -> Status {
    match err {
        ...
        | BucketError::DeleteRefusedNoProof { .. } => Status::failed_precondition(err.to_string()),
        BucketError::NothingToCreate => Status::invalid_argument(err.to_string()),
        BucketError::NameConflict { .. } => Status::already_exists(err.to_string()),
        BucketError::Admin { ref msg } => {
            error!(error = %msg, "rustfs admin error");
            Status::internal(err.to_string())
        }
        BucketError::Kube { .. } | BucketError::InvalidProof { .. } => {
            error!(error = %err, "bucket ownership store error");
            Status::internal(err.to_string())
        }
    }
}
...
fn map_grant(err: GrantError) -> Status {
    if err.is_access_key_immutable() {
    ...
    match err {
        GrantError::MissingExternalPolicy { .. } => Status::failed_precondition(err.to_string()),
        GrantError::Credentials { ref source } => {
            error!(error = %source, "credential store error");
            Status::internal(err.to_string())
        }
        GrantError::Ownership { ref source } => {
            error!(error = %source, "ownership store error");
            Status::internal(err.to_string())
        }
        GrantError::Admin { ref msg } => {
            error!(error = %msg, "rustfs admin error");
            Status::internal(err.to_string())
        }
...
impl Provisioner for Driver {
    async fn driver_create_bucket(
        ...
        let outcome = bucket::create_bucket(&self.kube, &client, &params, &req.name)
            .await
            .map_err(map_bucket)?;
    ...
    async fn driver_delete_bucket(
        ...
        bucket::delete_bucket(&self.kube, &client, &params, &req.bucket_id)
            .await
            .map_err(map_bucket)?;
    ...
    async fn driver_revoke_bucket_access(
        ...
        grant::revoke_bucket_access(&self.kube, &client, &params, &req.account_id)
            .await
            .map_err(map_grant)?;
