...
pub enum BackendError {
    ...
    #[snafu(display("secret key missing: {key}"))]
    MissingSecretKey { key: &'static str },
    #[snafu(display("secret key empty: {key}"))]
    EmptySecretKey { key: &'static str },
    #[snafu(display("secret key is not valid utf8: {key}"))]
    InvalidSecretKey { key: &'static str },
    ...
    #[snafu(display("configmap key missing: {key}"))]
    MissingCaData { key: &'static str },
    #[snafu(transparent)]
    ClientBuild {
        source: rustfs_admin::RustfsClientError,
    },
}
...
fn secret_value<'a>(secret: &'a Secret, keys: &[&'static str]) -> Result<&'a str, BackendError> {
    let data = secret
        .data
        .as_ref()
        .ok_or(BackendError::MissingSecretKey { key: keys[0] })?;
    for key in keys {
        if let Some(bytes) = data.get(*key) {
            let value = std::str::from_utf8(&bytes.0)
                .map_err(|_| BackendError::InvalidSecretKey { key })?;
            if value.is_empty() {
    ...
    Err(BackendError::MissingSecretKey { key: keys[0] })
}
...
pub async fn admin_client_from_params(
    ...
    if let (Some(cm_name), Some(cm_ns)) = (
        ...
        let ca = cm
            ...
            .ok_or(BackendError::MissingCaData { key: "ca.crt" })?;
