...
pub enum ParameterError {
    #[snafu(display("missing required parameter `{key}`"))]
    MissingRequired { key: &'static str },
    #[snafu(display("parameter `{key}` is empty"))]
    Empty { key: &'static str },
    #[snafu(display(
        "conflicting parameters: `bucketName` and `buckets` may not both be set; \
         use `buckets` alone (comma-separated) for multiple names"
    ))]
    ConflictingBucketParams,
}
...
impl BackendParameters {
    pub fn from_map(map: &HashMap<String, String>) -> Result<Self, ParameterError> {
        let bucket_name = optional(map, "bucketName");
        let buckets = optional(map, "buckets");
        if bucket_name.is_some() && buckets.is_some() {
            return Err(ParameterError::ConflictingBucketParams);
        }
...
/// Stable, DNS-1123-safe identifier for the RustFS endpoint a claim/proof
/// belongs to. Two BucketClasses/BucketAccessClasses pointing at different
/// endpoints must never collide on the same account_id/bucket_id.
pub fn backend_id(params: &BackendParameters) -> String {
...
#[cfg(test)]
mod tests {
    use super::{BackendParameters, ParameterError, backend_id, grant_policy_name};
    use std::collections::HashMap;
    ...
    #[test]
    fn conflicting_bucket_params_are_rejected() {
        let mut map = base_map();
        map.insert("bucketName".into(), "one".into());
        map.insert("buckets".into(), "two,three".into());
        let err = BackendParameters::from_map(&map).unwrap_err();
        assert!(matches!(err, ParameterError::ConflictingBucketParams));
    }

    #[test]
    fn backend_id_is_stable_and_endpoint_specific() {
        let a = BackendParameters::from_map(&base_map()).unwrap();
        let mut other_map = base_map();
        other_map.insert("endpoint".into(), "http://other-rustfs".into());
        let b = BackendParameters::from_map(&other_map).unwrap();

        assert_eq!(backend_id(&a), backend_id(&a));
        assert_ne!(backend_id(&a), backend_id(&b));
    }
}
